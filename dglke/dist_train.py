...
def construct_cmd_script(args):
    ...
    """
    total_client = (args.num_client_proc + args.async_proc_num) * get_machine_count(args.ip_config)
    cmd_str = '''#!/bin/bash
SERVER_ID_LOW=$1
SERVER_ID_HIGH=$2
...
--regularization_coef %f --num_client %d --push_step %d --test_num %d --async_proc_num %d --ent_topk %d --rel_topk %d --pre_sample %d --topk %d --dynamic_prefetch %d''' % (args.model_name, args.dataset, args.data_path, args.format,
                                            ...
                                            args.push_step, args.test_num, args.async_proc_num, args.ent_topk, args.rel_topk, args.pre_sample, args.topk, args.dynamic_prefetch)
    if args.test == True:
        cmd_str += ' --test'
