...
class KGEServer(KVServer):
    ...
    def _pull_handler(self, name, ID, target):
        ...
        """
        original_name = name[0:-6]
        data_tensor = target[name][ID]
        state_sum = target[original_name+'_state-data-'][ID]
        shape0 = data_tensor.shape[0]
        return th.cat((data_tensor, state_sum.reshape(shape0, 1)), 1)


    # 用于更新参数同步回传的参数变化值
    def _update_handler(self, name, ID, state, data, target):
        """ Row-Sparse Adagrad updater
        """
        
        original_name = name[0:-6]
        state_sum = target[original_name+'_state-data-']
        state_sum.index_add_(0, ID, state)
        target[name].index_add_(0, ID, data)
...
def start_server(args):
    ...
    """
    # print("before start server [port]")
    th.set_num_threads(NUM_THREAD)
    # print("before start server [read ip]")
