...
class NewBidirectionalOneShotIterator:
    ...
    def one_shot_iterator(dataloader, neg_chunk_size, neg_sample_size, is_chunked,
                          neg_head, num_nodes):
        while True:
            # print("[sampler generator start]")
            for pos_g, neg_g in dataloader:
                # print("step max_step", self.step, self.args_max_step)
                # if self.step !=0 and self.step % self.args_max_step == 0:
                #     print("break")
                #     self.step = 0
                #     break
