...
from ..utils import mergesort, merge_intersection
...
class KEModel(object):
    ...
    def pull_model(self, client, pos_g, neg_g, entity_topk, relation_topk, entity_topk_emb, relation_topk_emb, local_cache, synchronize):
        ...
        with th.no_grad():
            ...
            ent_compute_index = []
            rel_compute_index = []
            ent_compute_pull_idx = []
            rel_compute_pull_idx = []
            # entity_topk_emb = None
            # relation_topk_emb = None
            ent_pull_tensor = None
            rel_pull_tensor = None
            entity_data = th.tensor([],dtype=th.long)
            relation_data = th.tensor([],dtype=th.long)

            # start1 = time.time()
            if local_cache == True and synchronize == True:
                # entity_id_list = global_entity_id.tolist()
                # relation_id_list = relation_id.tolist()
                ent_pull_tensor, ent_compute_index, ent_topk_idx = mergesort(global_entity_id.tolist(), entity_topk.tolist())
                rel_pull_tensor, rel_compute_index, rel_topk_idx = mergesort(relation_id.tolist(), relation_topk.tolist())

                ent_pull_tensor = th.tensor(ent_pull_tensor, dtype=th.long)
                rel_pull_tensor = th.tensor(rel_pull_tensor, dtype=th.long)

            elif local_cache == True and synchronize == False:
                ent_pull_tensor, ent_compute_pull_idx, ent_compute_idx, ent_topk_idx = merge_intersection(global_entity_id.tolist(), entity_topk.tolist())
                rel_pull_tensor, rel_compute_pull_idx, rel_compute_idx, rel_topk_idx = merge_intersection(relation_id.tolist(), relation_topk.tolist())
            
                ent_pull_tensor = th.tensor(ent_pull_tensor, dtype=th.long)
                rel_pull_tensor = th.tensor(rel_pull_tensor, dtype=th.long)
            else:
                ent_pull_tensor = global_entity_id
                rel_pull_tensor = relation_id
            
            # time_step1 = time.time() - start1

            # print("ent_pull_tensor", ent_pull_tensor.shape)
            # print("rel_pull_tensor", rel_pull_tensor.shape)
            # print("entity_topk", entity_topk.shape)
            # print("relation_topk", relation_topk.shape)

            # start1 = time.time()
            if ent_pull_tensor.shape[0] > 0:
                entity_data, local1, remote1, ent_remote_ratio, ent_hit_ratio, ent_local_num, ent_remote_num = client.cache_pull(name='entity_emb', id_tensor=ent_pull_tensor, local_cache=local_cache)
            if rel_pull_tensor.shape[0] > 0:
                relation_data, local2, remote2, rel_remote_ratio, rel_hit_ratio, rel_local_num, rel_remote_num = client.cache_pull(name='relation_emb', id_tensor=rel_pull_tensor, local_cache=local_cache)

            # time_step2 = time.time() - start1

            # start1 = time.time()
            if local_cache == True and synchronize == True:
                self.entity_emb.emb[entity_id] = entity_data[ent_compute_index]
                self.relation_emb.emb[relation_id] = relation_data[rel_compute_index]

                entity_topk_emb = entity_data[ent_topk_idx]
                relation_topk_emb = relation_data[rel_topk_idx]

            elif local_cache == True and synchronize == False:
                if len(ent_compute_pull_idx) > 0:
                    self.entity_emb.emb[entity_id[ent_compute_pull_idx]] = entity_data
                if len(ent_compute_idx) > 0:
                    self.entity_emb.emb[entity_id[ent_compute_idx]] = entity_topk_emb[ent_topk_idx]
                if len(rel_compute_pull_idx) > 0:
                    self.relation_emb.emb[relation_id[rel_compute_pull_idx]] = relation_data
                if len(rel_compute_idx) > 0:
                    self.relation_emb.emb[relation_id[rel_compute_idx]] = relation_topk_emb[rel_topk_idx]

                ent_local_num += global_entity_id.shape[0]-ent_pull_tensor.shape[0]
                rel_local_num += relation_id.shape[0]-rel_pull_tensor.shape[0]
            else:
                self.entity_emb.emb[entity_id] = entity_data
                self.relation_emb.emb[relation_id] = relation_data

            # time_step3 = time.time() - start1
            # print("step1:", time_step1, "step2:", time_step2, "step3:", time_step3)
            return local1+local2, remote1+remote2, ent_remote_ratio, rel_remote_ratio, ent_hit_ratio, rel_hit_ratio, ent_local_num, ent_remote_num, rel_local_num, rel_remote_num, entity_topk_emb, relation_topk_emb
    ...
    # new code 
    def push_gradient(self, args, client, local_cache, synchronize):
        ent_hit = 0.0
        rel_hit = 0.0
        local_time = 0
        remote_time = 0
        with th.no_grad():
            l2g = client.get_local2global()
            for step, entity_id, entity_data in self.entity_emb.trace:
                grad = entity_data.grad.data
                global_entity_id =l2g[entity_id]
                
                local1, remote1 = client.cache_push(name='entity_emb', id_tensor=global_entity_id, data_tensor=grad, local_cache=local_cache)
                local_time += local1
                remote_time += remote1

            for step, relation_id, relation_data in self.relation_emb.trace:
                grad = relation_data.grad.data
                local1, remote1 = client.cache_push(name='relation_emb', id_tensor=relation_id, data_tensor=grad, local_cache=local_cache)
                local_time += local1
                remote_time += remote1
            ...
            #     remote2 = 0
            #     if args.ent_topk > 0:
            #         local1, remote1, ent_hit = client.cache_sync(name='entity_emb')
            #     if args.rel_topk > 0:
            #         local2, remote2, rel_hit = client.cache_sync(name='relation_emb')
            #     local_time += local1 + local2
            #     remote_time += remote1 + remote2

        self.entity_emb.trace = []
        self.relation_emb.trace = []

        return local_time, remote_time, ent_hit, rel_hit
